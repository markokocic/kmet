...
| **UTF-8 streaming decoder** | `TextDecoder({stream: true})` — correct multi-byte decoding across chunk boundaries | `(String. raw-bytes "UTF-8")` — replacement chars for split multi-byte sequences | ⚠️ (shell output is ASCII, no practical difference) |
