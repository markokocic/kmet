...
Field reports of Clojure-semantics bugs in Jolt (`jolt v0.8.5-36-gbac15682`,
threaded Chez 10.x, 2026-09), each with a minimal repro, the expected
Clojure/babashka behavior, and the kmet test it broke. All were discovered
by running kmet's test suite under Jolt (`jolt test`); bb/JVM is the
reference implementation (real Clojure semantics).
...
## JOLT-1 — `java.net.URI` accepts illegal characters the JDK rejects
...
;; jolt:       returns a URI whose .getHost is "not a url"
(let [u (java.net.URI. "https://not a url")]
  (.getHost u))
```

**Expected vs actual:** the JDK's single-arg `URI` ctor validates the
...
**Status:** open. Workaround in kmet: validate with a stricter check (the
`java.net.URI` multi-arg ctor also throws on Jolt for a *different* reason —
see the 7-arg ctor gap in jolt-port.md M-something) or regex the host.

---

## JOLT-2 — `clojure.edn/read-string` silently drops a trailing `@` after a token
...
**Status:** open. Workaround in kmet: none yet (a reader-level corruption
check must mirror JVM token rules).
...
## JOLT-5 — map destructuring of `& {:keys ...}` kwargs throws on an odd trailing argument; Clojure ignores it
...
(f 1 :a)   ;; bb: [1 nil]  — the odd trailing keyword is dropped, like Clojure
           ;; jolt: IllegalArgumentException "Don't know how to create ISeq from: clojure.lang.Keyword"
