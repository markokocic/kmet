...
;; ─── Line-level diff (pi: Diff.diffLines — Myers O(ND)) ────────────────────
...
(defn- group-ops
  "Group consecutive ops of the same type into parts
   {:type :eq|:del|:add :lines [str]} (pi: Diff.diffLines parts)."
  [ops]
  (reduce (fn [acc op]
...
(defn format-diff-lines
  ...
  (let [old-lines (drop-trailing-empty old-lines)
        ...
        parts (group-ops (concat prefix-eq (myers-ops old-changed new-changed) suffix-eq))
