(ns kmet.app.llm
  "LLM API dispatcher (pi: the per-wire api/* files dispatched from the
   ...
   the per-API request builders in kmet.app.api.*. The request builders and
   their shared machinery live in kmet.app.api.{shared,openai-completions,
  ...
  (:require
   ...
   [kmet.app.api.anthropic-messages :refer [anthropic-request]]
   [kmet.app.api.azure-openai-responses :refer [azure-request]]
   [kmet.app.api.bedrock-converse-stream :refer [bedrock-request]]
   [kmet.app.api.google-generative-ai :refer [google-request]]
   [kmet.app.api.google-vertex :refer [vertex-request]]
   [kmet.app.api.mistral-conversations :refer [mistral-request]]
   [kmet.app.api.openai-codex-responses :refer [codex-request]]
   [kmet.app.api.openai-completions :refer [openai-request]]
   [kmet.app.api.openai-responses :refer [responses-request]]
   [kmet.app.api.shared :refer [ambient-auth-available? effective-effort]]))
