      ...
      (if (>= i n)
        ...
        (let [c (subs line i (inc i))
          ...
          (cond
            ...
            ;; Link: [text](url)
            (= c "[")
            (let [close-b (.indexOf remaining "]")]
              (if (>= close-b 0)
                (let [paren (.indexOf remaining "(" close-b)]
                  (if (and (>= paren 0) (= paren (inc close-b)))
                    (let [close-p (.indexOf remaining ")" paren)]
                      (if (>= close-p 0)
