...
| Initial providers | **opencode-go, opencode (Zen), deepseek, github-copilot, openai, xai** (Phase 11 added the responses wire API + the last two; Phase 12 added **openai-codex, azure-openai-responses**) |
...
- More providers on the existing APIs (A.2 table): anthropic, google, groq,
  cerebras, openrouter, mistral, etc.
...
|---|---|
| `:openai-completions` | ant-ling, baseten, cerebras, cloudflare-workers-ai, fireworks (+), groq, huggingface, moonshotai, moonshotai-cn, nvidia, openrouter, qwen-token-plan, qwen-token-plan-cn, qwen-token-plan-individual, together, xiaomi, xiaomi-token-plan-ams, xiaomi-token-plan-cn, xiaomi-token-plan-sgp, zai, zai-coding-cn |
...
Phase 12; plus the post-merge additions mistral-medium-3.5, openrouter
`auto`/`fusion`, and the azure mirror of the openai models — done, Phase
12). **radius has no generator section** — it is a purely dynamic provider
(catalog from the gateway config + ModelsStore persistence). Highlights to
port:
