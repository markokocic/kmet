...
| Initial providers | **28** — the Phase 11/12 set (opencode-go, opencode, deepseek, github-copilot, openai, xai, openai-codex, azure-openai-responses) + Phase 13's 18 (anthropic, google, groq, cerebras, huggingface, moonshotai, xiaomi, qwen-token-plan, minimax, nvidia, openrouter, fireworks, vercel-ai-gateway, …) |
...
- **zai / zai-coding-cn, together, baseten, ant-ling** — their thinking
  formats (`:zai :together :baseten :ant-ling` + chat-template machinery)
  are Deferred A.3.
- **kimi-coding** — `forceAdaptiveThinking` needs the anthropic adaptive
  thinking branch (Deferred A.3 AnthropicMessagesCompat).
- **cloudflare-workers-ai / cloudflare-ai-gateway** — the catalog base URLs
  carry `{CLOUDFLARE_ACCOUNT_ID}/{CLOUDFLARE_GATEWAY_ID}` placeholders pi
  interpolates in cloudflare-auth; kmet has no placeholder support yet
  (works via models.edn base-url overrides).
...
|---|---|
| ~~`:openai-completions`~~ | ~~ant-ling, baseten, cerebras, cloudflare-workers-ai, fireworks (+), groq, huggingface, moonshotai, moonshotai-cn, nvidia, openrouter, qwen-token-plan, qwen-token-plan-cn, qwen-token-plan-individual, together, xiaomi, xiaomi-token-plan-ams, xiaomi-token-plan-cn, xiaomi-token-plan-sgp, zai, zai-coding-cn~~ — **Phase 13: groq, cerebras, huggingface, moonshotai(×2), nvidia, openrouter, qwen-token-plan(×3), xiaomi(×4), fireworks (+); deferred: ant-ling, baseten, together, zai(×2) (A.3 formats), cloudflare-workers-ai (URL placeholders)** |
| ~~`:anthropic-messages`~~ | ~~anthropic, cloudflare-ai-gateway (+), fireworks (+), kimi-coding, minimax, minimax-cn, vercel-ai-gateway~~ — **Phase 13: anthropic, minimax(×2), fireworks (+), vercel-ai-gateway; deferred: kimi-coding (adaptive thinking), cloudflare-ai-gateway (URL placeholders)** |
