...
| Question | Decision |
|---|---|
| Vendored data format | **EDN** (`src/kmet/ai/model_data/<provider>.edn`), generated by a bb task |
...
providers/data/*.json (generated)       src/kmet/ai/model_data/*.edn
...
```
src/kmet/ai/models.clj           records, registry, catalog loading, manifest
src/kmet/ai/model_data/          generated EDN catalogs (committed)
  opencode-go.edn
  opencode.edn
  deepseek.edn
  github-copilot.edn
...
`bb check-model-data` tasks + `test/kmet/ai/test_model_data.clj` (offline
...
5. Write `src/kmet/ai/model_data/<provider>.edn` + `manifest.edn`
...
- `bb check-model-data` + `test/kmet/ai/test_model_data.clj` re-run the
...
Covered by test/kmet/ai/test_models.clj (`test-calculate-cost`),
...
catalog at `src/kmet/ai/image_model_data/image-models.edn` (generated live
...
`test/kmet/ai/test_image_models.clj` — catalog load (45 models, required
...
- `test/kmet/ai/test_models.clj` — registry semantics (register/override/
  clear, get-model, default-model), catalog loading, EDN shape, `calculate-cost`.
- `test/kmet/ai/test_model_data.clj` — offline strict validation of
...
- `test/kmet/ai/test_llm.clj` — updated: URL construction per api,
  ...
  (`usage-with-cost`).
- `test/kmet/ai/test_auth.clj` — env precedence (auth.edn over env), pi env
...
- Implemented (Phase 6):
  ...
  - `test/kmet/ai/test_models.clj` — `load-models-config!` integration:
  ...
  - `test/kmet/ai/test_llm.clj` — `request-headers` (model + provider
...
- Implemented (Phase 9): `test/kmet/ai/test_auth.clj` — env trio,
...
- Implemented (Phase 10): `test/kmet/ai/test_oauth.clj` — device-code poll
