...
| M10 | `bb.edn` tasks (22: `run` + build/test/lint/format/nrepl/generate-…/pack-extension/…) | **DONE (test task, 2026-09-07):** `kmet.runner` is now host-aware and tolerant — every test namespace is required in a try; unloadable ones (babashka-internal requires like `babashka.classpath`/`babashka.classes`, `java.time.format.DateTimeFormatter` gaps, …) are reported and skipped, the rest run. Per-var `^:slow` split + per-var filters work on BOTH hosts (`jolt test` non-slow / `jolt test-ext` slow; bb.edn `:paths ["src" "test"]` supplies the roots under jolt). Engine: bb = per-var output capture + ref counters; jolt = `clojure.test/test-vars` with jolt's own process-wide `counters` atom read as before/after deltas (`jolt?` = `(find-var 'clojure.core/*jolt-version*)`). Currently on jolt: 945 fast tests (61 nss) + 19 slow tests (9 nss) run; 43 namespaces skipped as unloadable. Remaining M10 work: `jolt build` packaging, lint/format gates, model generators | mostly done for tests |
...
public fns when no test file exists). On bb/JVM (2026-09-10): **all 26
load and test green** — the cheshire → data.json swap unblocked the 6
libs that depended on M1. Jolt side: **18 of 26 load and run, 8 need
work** (M1, M3, M5 still gap the Jolt stdlib).
...
**bb/JVM: all green.** Jolt: 8 libs need work — M1 (json, jsonrpc, sse:
no JSON lib in Jolt stdlib), M3 (crypto, aws_sigv4: JVM crypto classes),
M5 (archive: java.util.zip). The cheshire → data.json swap removed the
